...
export function Profile() {
  ...
  const pwMut = useMutation({
    mutationFn: (d: ChangePasswordInput) =>
      changePassword({ current_password: d.current_password, new_password: d.new_password }),
    onSuccess: () => {
      pwForm.reset();
      setPwSuccess(true);
    ...
    <div className="max-w-lg space-y-8">
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm space-y-5">
        ...
        <form onSubmit={pwForm.handleSubmit((d) => pwMut.mutate(d))} className="space-y-4">
          ...
          {pwMut.isError && <p className="text-sm text-red-600">Incorrect current password</p>}
