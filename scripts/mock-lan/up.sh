...
# Explicit, not implicit-via-directory-basename: compose.mock-lan.yml no
# longer pins its own project name (see that file's header) so it can also
# be merged into the main dev stack's project via scripts/run.sh
# --with-mock-lan -- this script's own standalone identity now has to be
# stated here instead.
export COMPOSE_PROJECT_NAME=argus-mock-lan
COMPOSE="docker compose -f compose.mock-lan.yml --profile mock-lan"
...
Mock-LAN environment ready (project: $COMPOSE_PROJECT_NAME).
  ...
  Also usable merged into the main dev stack:
    ./scripts/run.sh client --with-mock-lan

  smoke test:  ./scripts/mock-lan/smoke-test.sh
  shell in:    COMPOSE_PROJECT_NAME=$COMPOSE_PROJECT_NAME $COMPOSE exec netshoot sh
  logs:        COMPOSE_PROJECT_NAME=$COMPOSE_PROJECT_NAME $COMPOSE logs -f snmpsim
