...
def test_login_with_bcrypt_password_upgrades_to_argon2(
    ...
    user = User(email=email, hashed_password=bcrypt_hash, is_active=True, full_name="test")
    db.add(user)
    db.commit()
    db.refresh(user)
...
def test_login_with_argon2_password_keeps_hash(client: TestClient, db: Session) -> None:
    ...
    user = User(email=email, hashed_password=argon2_hash, is_active=True, full_name="test")
    db.add(user)
    db.commit()
    db.refresh(user)
...
def test_non_superuser_cannot_list_users(
    ...
    """A regular (non-superuser) authenticated user must not be able to list all users."""
...
def test_admission_status_round_trip(
    ...
    """PATCH /users/{id} with admission_status must persist the updated value."""
    user = create_user(
        session=db,
        user_create=UserCreate(
            email=random_email(),
