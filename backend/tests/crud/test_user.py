...
def test_check_if_user_is_active_inactive(db: Session) -> None:
    email = random_email()
    password = random_lower_string()
    user_in = UserCreate(email=email, password=password, is_active=False, full_name=random_lower_string())
    user = crud.create_user(session=db, user_create=user_in)
...
def test_check_if_user_is_superuser(db: Session) -> None:
    email = random_email()
    password = random_lower_string()
    user_in = UserCreate(email=email, password=password, is_superuser=True, full_name=random_lower_string())
    user = crud.create_user(session=db, user_create=user_in)
...
def test_check_if_user_is_superuser_normal_user(db: Session) -> None:
    ...
    password = random_lower_string()
    user_in = UserCreate(email=username, password=password, full_name=random_lower_string())
    user = crud.create_user(session=db, user_create=user_in)
...
def test_get_user(db: Session) -> None:
    password = random_lower_string()
    username = random_email()
    user_in = UserCreate(email=username, password=password, is_superuser=True, full_name=random_lower_string())
    user = crud.create_user(session=db, user_create=user_in)
...
def test_authenticate_user_with_bcrypt_upgrades_to_argon2(db: Session) -> None:
    ...
    user = User(email=email, hashed_password=bcrypt_hash, full_name="test")
    db.add(user)
    db.commit()
    db.refresh(user)
